...
$sql = "SELECT id, date, purpose, price FROM spanding WHERE user_id=?  AND soft_delete=0 ORDER BY date DESC";
...
foreach ($data_by_date as $date => $purposes) {
  ...
  echo '
    ...
    <td class="amount" id="amount-' . $sno . '">' . $totalValue . '</td> 
    <td><i class="bi bi-pencil-square edit_btn_history"></i></td>
    <td><i class="bi bi-trash3 delete-btn_history" data-date="' . $date . '"></i></td>
