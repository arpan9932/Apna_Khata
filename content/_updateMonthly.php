...
class Monthlyedit extends Validation{
    private $conn;

    public function __construct($conn) {
        $this->conn = $conn;
    }
    public function edit_monthly($id,$user_id, $source, $amount){
          // Validation
         ...
         $sql="UPDATE `monthly_money` SET `source`='$source', `amount`='$amount' WHERE `id`='$id' AND `user_id`='$user_id'";
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_SESSION['user_id'])) {
        ...
        $monthlyedit = new Monthlyedit($conn); 

        // Correct the '$_POST['date']' typo and call the function
        $result = $monthlyedit->edit_monthly($_POST['id'],$user_id, $_POST['source'], $_POST['amount']);
